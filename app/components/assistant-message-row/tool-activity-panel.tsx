...
export const THINK_START = "<INTERNAL_THINK_START>";

export const THINK_END = "<INTERNAL_THINK_END>";
...
export function parseThinkingStream(content: string): ParsedContent {
...
export function resolveToolMeta(label: string): {
  title: string;
  description: string;
  raw: string;
} {
...
export function formatElapsed(activity: ToolActivity, now: number): string {
...
export function StatusGlyph({ status }: { status: ToolActivityStatus }) {
  if (status === "completed") {
    return (
      <span
