...
│   └── build-electron.ts     # 完整打包流程（编译 → next build → forge package）
...
| `pnpm electron:build` | 完整打包：编译 + `next build` + 复制静态资源 + Forge 打包到 `out/` |
| `pnpm electron:package` | 仅执行 Forge package（需先 `next build`） |
| `pnpm electron:make` | 生成各平台安装包（.exe / .dmg / .deb 等） |
...
```bash
# 一键打包（生成未压缩的可执行目录到 out/）
pnpm electron:build

# 生成安装包（.exe / .dmg / .deb / .rpm / .zip）
pnpm electron:make
```
